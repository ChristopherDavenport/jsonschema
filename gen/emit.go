...
func (e *emitter) emit() ([]byte, error) {
	...
	header := e.cfg.HeaderComment
	if header == "" {
		header = "Code generated by jsonschema-gen; DO NOT EDIT."
	}
...
func (e *emitter) emitStruct(f *jen.File, d *gotype.Decl) {
	...
	for _, emb := range d.Embeds {
		fields = append(fields, e.embedField(emb)) // embedded (anonymous) field
	}
	for _, fld := range d.Fields {
...
func (e *emitter) emitUnmarshal(f *jen.File, d *gotype.Decl) {
	...
	// When required-check emission is suppressed, drop the required set so decode
	// stays lenient; the method is then emitted only for the structural work
	// (interface-field dispatch, allOf embedding) that encoding/json cannot do.
	if e.cfg.OmitRequiredChecks {
		required = nil
	}
...
// embedField renders an embedded (anonymous) struct field, qualifying it with
// its import when the type lives in another package (Config.ExternalRef).
func (e *emitter) embedField(emb *gotype.TypeRef) *jen.Statement {
	if emb.Import != "" {
		return jen.Qual(emb.Import, emb.Named)
	}
	return jen.Id(emb.Named)
}
...
func (e *emitter) typeCode(t *gotype.TypeRef) *jen.Statement {
	...
	switch {
	case t.Named != "" && t.Import != "":
		// A cross-package named type (Config.ExternalRef): qualify with its import.
		base = jen.Qual(t.Import, t.Named)
	case t.Named != "":
		base = jen.Id(t.Named)
	case t.Import != "":
	...
	case "int64":
		if n, ok := v.(interface{ Int64() (int64, error) }); ok {
			if i, err := n.Int64(); err == nil {
				// Untyped literal so it is assignable to a defined integer enum
				// type (a typed int64(...) would not be).
				return jen.Id(strconv.FormatInt(i, 10))
			}
		}
		return jen.Lit(fmt.Sprint(v))
	case "float64":
